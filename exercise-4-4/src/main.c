...
static bool init(GLFWwindow *window)
{
	...
	glm_vec3_copy((vec3){ 0.0f, 7.0f, 15.0f }, cameraPos);
...
static void display(GLFWwindow *window, double currentTime)
{
	...
	ms_push(&matrixStack, sunMatPos);

	// Añadimos el planeta.

	mat4 planetMatPos = GLM_MAT4_IDENTITY_INIT;
	glm_translate(planetMatPos, (vec3){ 4.0f * sinf(t), 0.0f, 4.0 * cosf(t) });
	ms_add(&matrixStack, planetMatPos);

	// Añadimos la luna.

	mat4 moonMat = GLM_MAT4_IDENTITY_INIT;
	glm_translate(moonMat, (vec3){ 0.0f, 2.0f * sinf(t), 2.0f * cosf(t) });
	...
	mat4 vMat;
	glm_lookat(cameraPos, ms_top(&matrixStack)[3], (vec3){ 0.0f, 1.0f, 0.0f }, vMat);

	mat4 vpMat;
	glm_mat4_mul(pMat, vMat, vpMat);
	glUniformMatrix4fv(2, 1, GL_FALSE, vpMat[0]);
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(600, 600, "Capítulo 4 - Ejercicio 4", NULL, NULL);
	if (!window) {
