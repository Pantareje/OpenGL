...
static bool init(GLFWwindow *window)
{
	...
	glm_vec3_copy((vec3){ 30.0f, 0.0f, 30.0f }, cameraPos);
...
static void display(GLFWwindow *window, double currentTime)
{
	...
	glm_vec3_normalize_to((vec3){ +1.0f, +0.0f, -1.0f }, U);
	glm_vec3_normalize_to((vec3){ +0.0f, +1.0f, +0.0f }, V);
	glm_vec3_normalize_to((vec3){ +1.0f, +0.0f, +1.0f }, N);

	mat4 cameraRotation;
	glm_mat4_zero(cameraRotation);
	glm_vec3_copy(U, cameraRotation[0]);
	glm_vec3_copy(V, cameraRotation[1]);
	glm_vec3_copy(N, cameraRotation[2]);
	cameraRotation[3][3] = 1.0f;
	glm_mat4_transpose(cameraRotation);

	vec3 negCameraPos;
	glm_vec3_negate_to(cameraPos, negCameraPos);

	glm_mat4_identity(vMat);
	glm_mat4_mul(vMat, cameraRotation, vMat);
...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(600, 600, "Capítulo 4 - Ejercicio 2", NULL, NULL);
	if (!window) {
